...
const UI = (() => {
  ...
  function renderTable(items) {
    ...
    let html = `
      ...
      <thead>
        <tr>
          ...
          <th class="col-name">70迈商品名称</th>
          <th class="col-barcode">备件条码</th>
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const emptyActive = item['空包裹'] === true ? ' active' : '';

      html += `
        <tr class="${rowClass}" data-id="${item.recordId}">
          <td class="col-code">${escHtml(item['70迈料号'] || item['商品编号'] || '-')}</td>
          <td class="col-name">${escHtml(item['70迈商品名称'] || item['商品名称'] || '-')}</td>
          <td class="col-barcode">${escHtml(item['备件条码'] || '-')}</td>
          ...
          <td class="col-actions">
            ${!isDone ? `
              ...
              <button class="btn btn-outline btn-sm${emptyActive}" data-action="disc" data-flag="空包裹" data-id="${item.recordId}">空包裹</button>
    ...
    // Bind SN input save on blur/enter
